...
solar_constant<-function(yearday,S0=1360){
#declination <-   (23.45*sin(pi/180*360/365 * (284 + yearday)))*(pi/180) #1.3.3
phi <- 2*pi*(yearday-1)/365
#from chapter 1.3 An Introduction to Solar Radiation - Muhammad Iqbal
 day_angle<-(2*pi*(yearday-1))/365 #1.2.2
declination<-(0.006918-0.399912*cos(day_angle)+
                ...
                0.00148*sin(3*day_angle)) #1.3.1
#declination<-asin(0.4*sin(360/365*(yearday-82)))*(pi/180)  #1.3.2
...
global_radiation_quality_check_fast<-function(yearday,lat,lon,S0=1367){
sun_constants<-solar_constant(yearday)
#sun_rise_set<-suncalc(yearday,lat,lon)
...
#tan(lat.rad)*tan(declination)< -1 # continual night
#tan(lat.rad)*tan(declination)> 1 # continual day
     if(tan(lat.rad)*tan(declination) < -1){
		Qmax=0;Qmin=0
     }else if(lat==0){ 
     		H0 <- (24/pi)*Isc*d*sin(declination)
     		Qmax<-H0/(24*3600)
     		Qmin<-0.03*Qmax
     }else if(tan(lat.rad)*tan(declination) > 1){
     		H0 <- (24/pi)*Isc*d*sin(lat.rad)*sin(declination)*(pi) #4.2.22 in rad
     		Qmax<-H0/(24*3600)
     		Qmin<-0.03*Qmax
     }else{     
		omega <- acos(-tan(lat.rad)*tan(declination)) #1.5.3
		H0 <- (24/pi)*Isc*d*sin(lat.rad)*sin(declination)*(omega-tan(omega)) #4.2.19 in rad
                Qmax<-H0/(24*3600)
                Qmin<-0.03*Qmax}
     
return(list("yday"=yearday,"Qmax"=Qmax,"Qmin"=Qmin))
}
...
global_radiation_quality_check<-function(timedate,lat,lon,C=100,C_rare=50,alpha=1.2,alpha_rare=1.5){
  ...
  #solar_angles <- sunAngle(t=time,latitude = lat,longitude = lon) #only works between the years 1950-2050
  solar_angles <- sunPosition(time = day_seq,lat = lat,long = lon)
...
#'Solar position
#'@description Calculates the solar altitude and azimuth for a given time and location
#'@param time time of the day as POSIXct
#'@param lat latitude in WGS84
#'@param lon longitude in WGS84
#'@details requires namespace lubridate to extracts year, month, day, hour, minute, second from the POSIXct time. 
sunPosition <- function(time,lat=46.5, long=6.5) {
  requireNamespace("lubridate")
  ...
  day_time <- lubridate::day(time) + cumsum(month.days)[lubridate::month(time)]
  leapdays <- lubridate::year(time) %% 4 == 0 & (lubridate::year(time) %% 400 == 0 | lubridate::year(time) %% 100 != 0) & 
    day_time >= 60 & !(lubridate::month(time)==2 & day_time==60)
  day_time[leapdays] <- day_time[leapdays] + 1
  
  # Get Julian date - 2400000
  hour_time <- lubridate::hour(time) + lubridate::minute(time) / 60 + lubridate::second(time) / 3600 # hour plus fraction
  delta <- lubridate::year(time) - 1949
  leap <- trunc(delta / 4) # former leapyears
  jd <- 32916.5 + delta * 365 + leap + day_time + hour_time / 24
  ...
  gmst <- 6.697375 + .0657098242 * time + hour_time
  gmst <- gmst %% 24
  ...
  return(list(altitude=el, azimuth=az))
}
